...
if not exists("Cisco logs review"):
    mkdir("Cisco logs review")
        ...
        for line in file.readlines():
            ...
            for code in normal_code_list:
                if line.find(code) != -1:
                    ...
                    break
            if not is_normal:
                with open(join("Cisco logs review", file_list[file_index]), "a") as result_file:
...
# Summary
print("\n\nSummary:")
summary_file = open(join("Cisco logs review", "Summary " + the_month + ".txt"), "w")
